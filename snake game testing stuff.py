...
def wrap_graph_edge(v, max):
    """
    For snake to go thru walls
    Allows edges to wrap around the board
    """
    if v < 0:
        return max
    elif v >= max:
...
def should_draw_line(point_a, point_b):
    """
    When drawing path to food thru walls
    """
